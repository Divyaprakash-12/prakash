...
/*6.Show the working of Merge Statement by creating a backup for the students details of only students in Sem 1.
Note: Update few values in students details so that you can see the working of UPDATE.*/
set identity_insert sqltask8.dbo.student on

select * into sem1backup from student where semester='1 sem';

select * from sem1backup--target table
select * from student--source table
...
merge sem1backup as s1 using (select * from student where semester='1 sem')  as s on (s1.student_id=s.student_id) 
when matched
then update set
...
select * from student
>>>>>>> a3c5f4d7d1516f70ae1e0bc6013c15faf08c7177
